...
class Device(models.Model):
    ...
    def __str__(self):
        ...
        return_text = _('OTP Device')
        return f'{return_text}-{user} '
...
class SideChannelDevice(Device):
    ...
    """
    token = models.CharField(max_length=16, blank=True, null=True, verbose_name=_('token'))
    valid_until = models.DateTimeField(default=timezone.now,
                                       help_text=_('The timestamp of the moment of expiry of the saved token.'))
...
class ThrottlingMixin(models.Model):
    ...
    """
    throttling_failure_timestamp = models.DateTimeField(null=True, blank=True, default=None,
                                                        verbose_name=_('Last failed attempt'))
    throttling_failure_count = models.PositiveIntegerField(default=0, verbose_name=_('Number of attempts'))
    ...
    def verify_is_allowed(self):
        ...
        """
        if (self.throttling_enabled and self.throttling_failure_count > 0 and
