...
class RatingManager(models.Manager):
    ...
    @staticmethod
    def delete_existing(existing_rating):
...
class AbstractBaseRating(models.Model):
    ...
    average = models.DecimalField(_('average'), max_digits=6, decimal_places=3, default=Decimal(0.0))

    content_type = models.ForeignKey(ContentType, null=True, blank=True, on_delete=models.CASCADE,
                                     verbose_name=_('content type'))
    object_id = models.PositiveIntegerField(_('id'), null=True, blank=True)
...
class UserRating(TimeStampedModel):
    class Meta:
        ...
        verbose_name_plural = verbose_name = _('User rating')
    ...
    rating = models.ForeignKey(get_star_ratings_rating_model_name(), verbose_name=_('content'),
                               related_name='user_ratings', on_delete=models.CASCADE)
    ...
    def __str__(self):
        ...
        dote_ = _(".")
        if not app_settings.STAR_RATINGS_ANONYMOUS:
            return f'{self.user}: {score_} {self.score} {for_} "{self.rating.content_object}" {dote_}'
        return f'{self.ip}: {score_} {self.score} {for_} "{self.rating.content_object}" {dote_}'